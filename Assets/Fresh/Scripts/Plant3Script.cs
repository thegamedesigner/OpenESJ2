...
public class Plant3Script : MonoBehaviour
{
	...
	void Start()
	{
		...
		flower1.transform.localScale = new Vector3(0,0,1);

		startingDelay = Random.Range(0, 0.7f);

		seedAngle = Random.Range(-5, 5);
	...
	void Update()
	{
		...
		//flower1.transform.SetX(branch3Joint.transform.position.x);
		//flower1.transform.SetY(branch3Joint.transform.position.y);
		switch (state)
		{
