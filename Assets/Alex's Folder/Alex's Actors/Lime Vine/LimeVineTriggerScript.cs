...
public class LimeVineTriggerScript : MonoBehaviour {

	public int limeTargetPoint = 0;
	float  plBoxHeight = 0f, plBoxWidth = 0f;
	bool inMe = false, triggered = false;
	public LimeVineControllerScript vineScript = null;
	...
	void Update () {
		...
		if(xa.player){
			...
			if(inMe && !triggered){
				vineScript.maxVinePointIndex = limeTargetPoint;
				triggered = true;
			}
		}
	}
